...
localdir="displayotron" # the name of the dir for copy of examples
...
examplesdir="$gitreponame/python/examples" # subdirectory of examples in repo 
...
moreaptdep=( "libudev-dev" "vlc" ) # list of all the additional apt dependencies
morepipdep=( "psutil" "python-uinput" "wifi" ) # list of all the additional pip dependencies
...
if confirm "Do you wish to continue?"; then
    ...
    if [ $spireq == "yes" ]; then
        ...
        if confirm "Hardware requires SPI, enable now?"; then
            echo MAKE SURE YOU ENABLE IT WITH RASPI-CONFIG !!!!
        fi
    fi
